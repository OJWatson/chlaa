...
public:
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    state[28] = 0;
    state[29] = 0;
    state[30] = shared.Du0 + shared.Dt0;
    state[31] = 0;
    state[32] = 0;
    state[33] = 0;
    state[34] = 0;
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto expected_cases = state[26];
    const auto expected_cases_weekly = state[27];
    const auto cum_infections = state[28];
    const auto cum_symptoms = state[29];
    const auto cum_deaths = state[30];
    const auto cum_vax1 = state[31];
    const auto cum_vax2 = state[32];
    const auto cum_orc_treated = state[33];
    const auto cum_ctc_treated = state[34];
    const auto cum_expected_cases = state[35];
    ...
    state_next[26] = expected_cases + shared.reporting_rate * new_symp;
    state_next[27] = expected_cases_weekly + shared.reporting_rate * new_symp;
    state_next[28] = cum_infections + new_E;
    state_next[29] = cum_symptoms + new_symp;
    state_next[30] = cum_deaths + death_Sevu + death_Sevt;
    state_next[31] = cum_vax1 + vax1_admin;
    state_next[32] = cum_vax2 + vax2_admin;
    state_next[33] = cum_orc_treated + treat_orc;
    state_next[34] = cum_ctc_treated + treat_ctc;
    state_next[35] = cum_expected_cases + shared.reporting_rate * new_symp;
  }
  static auto zero_every(const shared_state& shared) {
    return dust2::zero_every_type<real_type>{{1, {16}}, {1, {17}}, {1, {18}}, {1, {19}}, {1, {20}}, {7, {21}}, {7, {22}}, {7, {23}}, {7, {24}}, {7, {25}}, {1, {26}}, {7, {27}}};
  }
  static real_type compare_data(real_type time, const real_type* state, const data_type& data, const shared_state& shared, internal_state& internal, rng_state_type& rng_state) {
    ...
    const auto expected_cases_weekly = state[27];
    real_type odin_ll = 0;
    const real_type obs_expected_cases = (data.obs_interval <= static_cast<real_type>(1.5) ? expected_cases : expected_cases_weekly);
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.cases, shared.obs_size, obs_expected_cases, true));
    return odin_ll;
  }
};
