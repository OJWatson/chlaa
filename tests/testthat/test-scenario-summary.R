test_that("standard scenarios include required templates", {
  ...
  sc <- cholera_standard_scenarios(pars, trigger_time = 30, horizon = 180)
...
test_that("scenario summary and report return expected outputs", {
  ...
  pars <- cholera_parameters()
  sc <- cholera_standard_scenarios(pars, trigger_time = 20, horizon = 100, vax_total_doses = 20000)
  runs <- cholera_run_scenarios(pars, sc, time = 0:30, n_particles = 3, dt = 1, seed = 1)

  summ <- cholera_scenario_summary(runs, baseline = "baseline")
  ...
  rep <- cholera_scenario_report(runs, baseline = "baseline", include_econ = TRUE, wtp = 1000)
